...
class User(db.Model):
    ...
    credit = db.Column(db.Integer, default=0)  # Added credit column
    download_history = db.relationship('DownloadHistory',
                                       backref='user',
                                       lazy=True)
...
def signup():
    if request.method == 'POST':
        ...
        # Assign 5 credits to the new user
        new_user.credit = 5
        db.session.commit()
...
def convert_to_mp3():
    ...
    if user.download_count >= download_limit:
        ...
        return render_template(
            'index.html',
            error=
            "Download limit reached. You can't download more than 5 files.",
            remaining_downloads=remaining_downloads)
    ...
    try:
        ...
        user.credit -= 1  # Deduct 1 credit for each conversion
        db.session.commit()
...
def buycredit():
    ...
    user_id = session['user_id']
    user = User.query.get(user_id)
    return render_template('buycredit.html',
                           username=user.username,
                           credit_remaining=user.credit)
